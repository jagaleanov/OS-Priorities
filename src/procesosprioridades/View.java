...
public class View extends JFrame implements ActionListener, Runnable {
    ...
    public View() {
        ...
        this.columnNamesQueue = new String[]{"Id", "Llegada", "Ráfaga", "Prioridad"};
        this.columnNamesBlock = new String[]{"Id", "Llegada", "Ráfaga", "Prioridad", "Ráfaga ejecutada"};
        this.columnNamesHistory = new String[]{"Id", "Llegada", "Ráfaga ejecutada", "Prioridad", "Comienzo", "Final", "Retorno", "Espera"};
        this.columnNamesGantt = new String[]{"Id"};

        setBounds(0, 50, 1100, 300);
        setTitle("Gestión de procesos no expulsivos");
        ...
        panelTop.add(new JLabel("Prioridad (1,2,3)"));
        this.txtPriority = new JTextField("", 5);
        panelTop.add(this.txtPriority);//Prioridad
        ...
        this.tableGantt.setDefaultRenderer(Object.class, new Render());
    ...
    public void addProcess() {
        ...
        try {
            ...
            int priority = Integer.parseInt(this.txtPriority.getText());

            if (priority > 0 && priority <= 3) {
                this.queue.add(this.time, raf, priority);
                this.tableModelQueue.setRowsQueue(this.queue.getHead());
                this.tableModelGantt.addRowGantt(this.queue.getTail(), this.time);

                this.txtRaf.setText("");
            } else {
                JOptionPane.showMessageDialog(this, "La prioridad debne ser 1, 2 o 3");
            }
        } catch (Exception e) {
                ...
                if (this.process != null) {//si hay un proceso en estado critico
                    ...
                    if (this.process.getRaf() <= this.process.getRafExecuted()) {//si ya termino su rafaga
                        ...
                        this.lblStatus.setText("Estado: Atendiendo proceso: " + this.process.getId() + ", "
                                ...
                                + "Prioridad: " + this.process.getPriority());
                    }
                }
